...
export class ComplianceService {
    ...
    async generateEsiEcr(payrollRunId: string): Promise<string> {
        ...
        const lines = run.items
            ...
            .map((item) => {
                ...
                const name = item.employee.firstName + ' ' + item.employee.lastName;
                const gross = Math.floor((item.netPay + (item.deductions as any).lop) / 100);
    ...
    async auditPayroll(payrollRunId: string) {
        ...
        for (const item of run.items) {
            ...
            if (declaration && declaration.regime === TaxRegime.NEW && deductions.tds > 10000 && monthlyGross < 70000) {
                anomalies.push({
                    employeeId: item.employeeId,
                    name: `${item.employee.firstName} ${item.employee.lastName}`,
